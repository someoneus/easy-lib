...
hide.TouchTap:Connect(function()
hide.Visible = false
window.Visible = true
end)
local frame = window
...
function easy.Components:CreateButton(options)
    if lastCreatedFrame then
        ...
        local anim = tween:Create(button, info, goal)
...
function easy.Components:CreateInput(txtoption)
    if txtoption then
        ...
        textb.Text = txtoption.Placeholder
        textb.PlaceholderText = txtoption.Placeholder
...
function easy.Components:CreateToggle(option)
    ...
    ui.Parent = out
    ui.Thickness = 3.5
    ui.Color = Color3.fromRGB(90, 90, 90)
    ui.Transparency = 0
    local ui = Instance.new("UICorner")
    ui.Parent = out
    ui.CornerRadius = UDim.new(0.5, 0)
    ...
    local anim = tween:Create(toggle, info, goal)
    ...
    local anim2 = tween:Create(toggle, info2, goal2)
    local Value = false
